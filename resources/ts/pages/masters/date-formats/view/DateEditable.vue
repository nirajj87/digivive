...
const onSubmit = () => {
  ...
  if (platformId) {
    ...
    dateListStore.update(props.platformData)
      .then((response: any) => {
    ...
    props.platformData.status = props.platformData.status
    dateListStore.add(props.platformData)
      .then((response: any) => {
        isSnackbarVisibileDialog.value = true
        snackbarClass.value = 'success-snackbar'
        message.value = response.data.message;
      ...
      <VForm ref="refForm" v-model="isFormValid" @submit.prevent="onSubmit">
          ...
          <VCol cols="12">
            <VCard border>
              <VCardText>
                <VRow>
                  <VCol cols="12" md="4">
                    <VTextField :label="$t('date')" :rules="[requiredValidator]" v-model="platformData.title" />
                  </VCol>
                  <VCol cols="12" md="4">
                    <VTextField :label="$t('date_format')" :rules="[requiredValidator]"
                      v-model="platformData.format_key" />
                  </VCol>
                  <VCol cols="12" md="4">
                    <VTextField :label="$t('order')" :rules="[requiredValidator]" v-model="platformData.order" />
                  </VCol>
                  <VCol cols="12" md="4">
        ...
        <VRow>
          <VCol cols="12" class="text-right">
            <VBtn type="submit" color="secondary mr-3" :to="{ name: 'masters-date-formats' }">
              {{ $t("can_btn") }}
            </VBtn>
