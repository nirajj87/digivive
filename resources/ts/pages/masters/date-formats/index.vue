...
import type { DateList } from '@/pages/masters/date-formats/view/types';
...
const openDialog = (date: any) => {
  deleteId.value = date.id;
  expectedValue.value = date.title;
...
// Delete
const deleteModules = () => {
  dateListStore.delete(deleteId.value)
    .then((response: any) => {
...
const breadcrumbsData = ref({
  page_name: 'date_format',
  name: 'date_format',
});

const paginationData = computed(() => {
  const firstIndex = dateList.value.length ? ((currentPage.value - 1) * rowPerPage.value) + 1 : 0
  const lastIndex = dateList.value.length + ((currentPage.value - 1) * rowPerPage.value)
                ...
                <VCol cols="9">
                  <VRow>
                    ...
                    <VCol cols="12" md="3">
                      ...
                      <VBtn prepend-icon="tabler-plus" :to="{ name: 'masters-date-formats-add' }">
                        {{ $t("add") }}
                      </VBtn>
                    </VCol>
                  </VRow>
              ...
              <VTable v-show="!isLoading" class="text-no-wrap table-background">
                ...
                <tbody v-if="!isLoading">
                  ...
                  <tr v-for="date in dateList" :key="date.id">
                    ...
                    <td>
                      <VChip label
                        :color="date?.status === '1' ? ($t('success')) : (date?.status === '0' ? ($t('error')) : ($t('blocked')))"
                        size="small">
                        {{ date?.status === '1' ? ($t('active')) : (date?.status === '0' ? ($t('inactive')) :
                          ($t('blocked')))
                        }}
                      </VChip>
                    </td>
                      ...
                      <VBtn icon variant="text" color="default" size="x-small">
                        ...
                        <VMenu activator="parent">
                          <VList>
                            ...
                            <VListItem :to="{ name: 'masters-date-formats-details-id', params: { id: date.id } }">
                              <template #prepend>
                            ...
                            <VListItem :to="{ name: 'masters-date-formats-edit-id', params: { id: date.id } }">
                              <template #prepend>
                            ...
                            <VListItem @click="openDialog(date)">
                              <template #prepend>
