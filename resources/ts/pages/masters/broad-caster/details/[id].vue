...
import type { BroadCasterList } from './../view/types';
import { useBroadCasterStore } from './../view/useBroadCasterStore';
                ...
                <VCardText>
                        ...
                        <VCardText>
                            <VRow>
                                <VCol cols="12" md="4">
                                    ...
                                    <h5>{{ broadList?.title }}</h5>
                                </VCol>
                                <VCol cols="12" md="4">
                                    <p class="mb-0">{{ $t("slug") }}</p>
                                    <h5>{{ broadList?.slug }}</h5>
                                </VCol>
                                <VCol cols="12" md="4">
                                    <p class="mb-0">{{ $t("status") }}</p>
                                    <h5>{{ broadList?.status == '1' ? ($t('active')) : ($t('inactive')) }}</h5>
                                </VCol>
                                ...
                                <VCol cols="12" class="text-center" v-if="isLoading">
                                    ...
                                    <span v-show="!broadList && !isLoading">{{ $t('no_data_found') }}</span>
                                </VCol>
                    ...
                    <VRow class="mt-2">
                        <VCol cols="12" class="text-right">
                            <VBtn prepend-icon="tabler-chevron-left" :to="{ name: 'masters-broad-caster' }">
                                {{ $t("back") }}
                            </VBtn>
                        </VCol>
                    </VRow>
