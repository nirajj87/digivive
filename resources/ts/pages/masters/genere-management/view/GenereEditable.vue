...
const onSubmit = () => {
  ...
  if (platformId) {
    ...
    genereListStore.update(props.platformData)
      .then((response: any) => {
        ...
        setTimeout(() => {
          router.push({
            name: "masters-genere-management"
          });
        }, 2000);
    ...
    props.platformData.status = props.platformData.status
    genereListStore.add(props.platformData)
      .then((response: any) => {
        isSnackbarVisibileDialog.value = true
        snackbarClass.value = 'success-snackbar'
        message.value = response.data.message;
    ...
    <VForm ref="refForm" v-model="isFormValid" @submit.prevent="onSubmit">
          ...
          <VCard border>
            <VCardText>
              <VRow>
                ...
                <VCol cols="12" md="4">
                  <VTextField :label="$t('user_id')"  :rules="[requiredValidator]" v-model="platformData.user_id" />
                </VCol>
                <VCol cols="12" md="4">
      ...
      <VRow>
        <VCol cols="12" class="text-right">
          <VBtn type="submit" color="secondary mr-3" :to="{ name: 'masters-genere-management' }">
            {{ $t("can_btn") }}
          </VBtn>
