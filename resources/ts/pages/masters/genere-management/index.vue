...
import type { GenereList } from '@/pages/masters/genere-management/view/types';
...
const openDialog = (genere: any) => {
  deleteId.value = genere.id;
  expectedValue.value = genere.title;
...
// Delete
const deleteModules = () => {
  genereListStore.delete(deleteId.value)
    .then((response: any) => {
...
const paginationData = computed(() => {
  const firstIndex = genereList.value.length ? ((currentPage.value - 1) * rowPerPage.value) + 1 : 0
  const lastIndex = genereList.value.length + ((currentPage.value - 1) * rowPerPage.value)
              ...
              <VCol cols="9">
                <VRow>
                  ...
                  <VCol cols="12" md="3">
                    ...
                    <VBtn prepend-icon="tabler-plus" :to="{ name: 'masters-genere-management-add' }">
                      {{ $t("add") }}
                    </VBtn>
                  </VCol>
                </VRow>
          ...
          <VCardText class="pt-0 pb-0">
            ...
            <VTable v-show="!isLoading" class="text-no-wrap table-background">
              ...
              <tbody v-if="!isLoading">
                ...
                <tr v-for="genere in genereList" :key="genere.id">
                  ...
                  <td>
                    <VChip label
                      :color="genere?.status === '1' ? ($t('success')) : (genere?.status === '0' ? ($t('error')) : ($t('blocked')))"
                      size="small">
                      {{ genere?.status === '1' ? ($t('active')) : (genere?.status === '0' ? ($t('inactive')) :
                        ($t('blocked'))) }}
                    </VChip>
                  </td>
                    ...
                    <VBtn icon variant="text" color="default" size="x-small">
                      ...
                      <VMenu activator="parent">
                        <VList>
                          ...
                          <VListItem :to="{ name: 'masters-genere-management-details-id', params: { id: genere.id } }">
                            <template #prepend>
                              <VIcon size="24" class="me-3"
                                icon="tabler-eye" />
                            </template>
                            <VListItemTitle>{{ $t("view") }}</VListItemTitle>
                          </VListItem>

                          <VListItem :to="{ name: 'masters-genere-management-edit-id', params: { id: genere.id } }">
                            <template #prepend>
                          ...
                          <VListItem @click="openDialog(genere)">
                            <template #prepend>
                ...
                <ConfirmDialog v-model:isDialogVisible="isConfirmDialogOpen"
                  :confirmation-msg="$t(`${dialogueBoxMessage}`)" @confirm="deleteModules()" v-model:expected-value="expectedValue" />
              </tbody>

              <tfoot v-show="!genereList.length">
                <tr>
