...
const openDialog = (viewer: any) => {
  deleteId.value = viewer.id;
  expectedValue.value = viewer.title;
...
// Delete
const deleteModules = () => {
  viewerListStore.delete(deleteId.value)
    .then((response: any) => {
...
const breadcrumbsData = ref({
  page_name: 'viewer_rating',
  name: 'viewer_rating',
});

const paginationData = computed(() => {
  const firstIndex = viewerList.value.length ? ((currentPage.value - 1) * rowPerPage.value) + 1 : 0
  const lastIndex = viewerList.value.length + ((currentPage.value - 1) * rowPerPage.value)
              ...
              <VCol cols="9">
                <VRow>
                  ...
                  <VCol cols="12" md="3">
                    ...
                    <VBtn prepend-icon="tabler-plus" :to="{ name: 'masters-viewer-rating-add' }">
                      {{ $t("add") }}
                    </VBtn>
                  </VCol>
                </VRow>
          ...
          <VCardText class="pt-0 pb-0">
            ...
            <VTable v-show="!isLoading" class="text-no-wrap table-background">
              ...
              <tbody v-if="!isLoading">
                ...
                <tr v-for="viewer in viewerList" :key="viewer.id">
                  ...
                  <td class="text-center">
                    <VChip label
                      :color="viewer?.status === '1' ? ($t('success')) : (viewer?.status === '0' ? ($t('error')) : ($t('blocked')))"
                      size="small">
                      {{ viewer?.status === '1' ? ($t('active')) : (viewer?.status === '0' ?
                        ($t('inactive')) :
                        ($t('blocked'))) }}
                    </VChip>
                  </td>
                    ...
                    <VBtn icon variant="text" color="default" size="x-small">
                      ...
                      <VMenu activator="parent">
                        <VList>
                          ...
                          <VListItem :to="{ name: 'masters-viewer-rating-details-id', params: { id: viewer.id } }">
                            <template #prepend>
                          ...
                          <VListItem :to="{ name: 'masters-viewer-rating-edit-id', params: { id: viewer.id } }">
                            <template #prepend>
                          ...
                          <VListItem @click="openDialog(viewer)">
                            <template #prepend>
              ...
              <tfoot v-show="!viewerList.length">
                <tr>
